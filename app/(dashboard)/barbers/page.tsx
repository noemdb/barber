        ...
        {items.map((b) => (
          <div key={b.id} className="rounded-2xl border bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between">
              <div
                className="h-12 w-12 rounded-2xl bg-zinc-100 bg-cover bg-center grid place-items-center overflow-hidden"
                role="img"
                aria-label={`Avatar de ${b.name}`}
                style={b.avatar ? { backgroundImage: `url(${b.avatar})` } : undefined}
              >
                {!b.avatar && <Scissors size={18} />}
              </div>
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => startEditing(b)} className="h-8 w-8 grid place-items-center rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-950" aria-label={`Editar ${b.name}`} title="Editar">
                  <Pencil size={15} />
                </button>
                <button type="button" onClick={() => remove(b)} className="h-8 w-8 grid place-items-center rounded-lg text-zinc-500 hover:bg-red-50 hover:text-red-700" aria-label={`Desactivar ${b.name}`} title="Desactivar">
                  <Trash2 size={15} />
                </button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/40 grid place-items-center p-4" onMouseDown={() => setOpen(false)}>
          <form onSubmit={save} onMouseDown={(e) => e.stopPropagation()} className="w-full max-w-md bg-white rounded-2xl p-5">
            <h2 className="font-semibold text-lg">{editing ? "Editar barbero" : "Nuevo barbero"}</h2>
            <div className="mt-5 space-y-4">
              {([["Nombre", "name"], ["Teléfono", "phone"], ["Correo", "email"], ["Especialidad", "specialty"]] as const).map(([label, key]) => (
                ...
                <label className="block text-xs font-medium">
                  Avatar
                  <div className="mt-2 flex items-center gap-4 rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 p-3">
                    <div className="relative h-20 w-20 shrink-0 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
                      {form.avatar ? (
                        <div
                      ...
                      {form.avatar && (
                        <button
                          ...
                          className="absolute right-1 top-1 grid h-6 w-6 place-items-center rounded-full bg-zinc-950/80 text-white hover:bg-zinc-950"
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 text-xs text-zinc-500">PNG, JPG, GIF o SVG · máximo 2 MB</p>
                      <UploadButton
                      ...
                      />
                      {uploading ? (
                        <p className="mt-1 flex items-center gap-1 text-xs text-zinc-500">Subiendo imagen...</p>
                      ) : form.avatar ? (
                        <p className="mt-1 flex items-center gap-1 text-xs text-emerald-700"><Check size={13} /> Avatar listo</p>
                      ) : null}
                    </div>
                  </div>
                </label>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button type="button" onClick={() => { setOpen(false); setEditing(null); }} className="h-10 px-4 border rounded-xl text-sm">Cancelar</button>
