...
function ClientDetails({ client, onClose }: { client: Client; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 dark:bg-black/60 p-4 backdrop-blur-[2px]" onMouseDown={onClose}>
      <div role="dialog" aria-modal="true" aria-labelledby="client-details-title" onMouseDown={(event) => event.stopPropagation()} className="w-full max-w-md rounded-2xl bg-white dark:bg-zinc-900 p-5 shadow-xl border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="mt-6 space-y-3 rounded-xl bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-100 dark:border-zinc-800 p-4 text-sm text-zinc-900 dark:text-zinc-100">
          <div className="flex items-center gap-3"><Phone size={15} className="text-zinc-400 dark:text-zinc-500" /><span>{client.phone || "Sin teléfono registrado"}</span></div>
          <div className="flex items-center gap-3"><Mail size={15} className="text-zinc-400 dark:text-zinc-500" /><span>{client.email || "Sin correo registrado"}</span></div>
        </div>
        {client.notes && <div className="mt-4"><p className="text-xs font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Notas</p><p className="mt-2 rounded-xl border border-zinc-100 dark:border-zinc-800 p-3 text-sm text-zinc-600 dark:text-zinc-400">{client.notes}</p></div>}
        <div className="mt-5 flex justify-end"><button type="button" onClick={onClose} className="h-10 rounded-xl bg-zinc-950 dark:bg-gold px-4 text-sm font-semibold text-white dark:text-zinc-950 hover:bg-zinc-800 dark:hover:bg-gold-light transition-colors">Cerrar</button></div>
      </div>
    </div>
  );
}
