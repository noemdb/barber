...
import { ArrowUpRight, CalendarDays, CalendarClock, CheckCircle2, CircleDollarSign } from "lucide-react";
...
export default async function BarberClientsPage({
  ...
  // ── Agregados del filtro (clientes distintos + KPIs) ─────────────────
  const [settings, totalAppointments, totalAmount, clientGroups] = await Promise.all([
  ...
  const recurring = clientGroups.filter((c) => c._count._all >= 2).length;

  // ── Paginación en memoria sobre clientes distintos ───────────────────
  const totalPages = Math.max(1, Math.ceil(totalClients / PAGE_SIZE));
  const clampedPage = Math.max(1, Math.min(currentPage, totalPages));
  const pageGroups = clientGroups.slice((clampedPage - 1) * PAGE_SIZE, clampedPage * PAGE_SIZE);

  const clientRecords = await prisma.client.findMany({
    where: { id: { in: pageGroups.map((g) => g.clientId) } },
    select: { id: true, name: true, phone: true, email: true },
  });
  const clientById = new Map(clientRecords.map((c) => [c.id, c]));
  const rows = pageGroups
    .map((g) => ({ ...g, client: clientById.get(g.clientId) }))
    .filter((r) => r.client);
  ...
  const pageHref = (page: number) => {
    ...
    const qs = p.toString();
    return qs ? `/barber/clients?${qs}` : "/barber/clients";
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Clientes con citas contigo · {rangeLabel} · {totalClients} clientes
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <Stat icon={CalendarDays} label="Clientes" value={String(totalClients)} trend="En el filtro seleccionado" />
        <Stat icon={CalendarClock} label="Citas" value={String(totalAppointments)} trend="De este periodo" />
        <Stat icon={CheckCircle2} label="Recurrentes" value={String(recurring)} trend="Con 2 o más citas" />
      ...
      <section className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-[820px] text-left">
            <thead className="bg-zinc-50 dark:bg-zinc-900 text-[10px] uppercase tracking-wide text-zinc-400 dark:text-zinc-500">
              <tr>
                ...
                <th className="px-5 py-3 text-zinc-500 dark:text-zinc-400">Contacto</th>
                <th className="px-5 py-3 text-zinc-500 dark:text-zinc-400">Citas</th>
                <th className="px-5 py-3 text-zinc-500 dark:text-zinc-400">Última cita</th>
                <th className="px-5 py-3 text-right text-zinc-500 dark:text-zinc-400">Monto</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.clientId} className="border-t border-zinc-100 dark:border-zinc-800 text-xs text-zinc-900 dark:text-zinc-100">
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className="grid h-8 w-8 place-items-center rounded-full bg-zinc-100 dark:bg-zinc-800 text-[10px] font-bold text-zinc-700 dark:text-zinc-300">
                  ...
                  <td className="px-5 py-3 text-zinc-600 dark:text-zinc-400">
                    ...
                    <div className="text-xs text-zinc-500 dark:text-zinc-500">{r.client!.email}</div>
                  </td>
                  <td className="px-5 py-3 text-zinc-600 dark:text-zinc-400">{r._count._all}</td>
                  <td className="px-5 py-3 text-zinc-600 dark:text-zinc-400">
                    {r._max.startsAt ? tzFormat(r._max.startsAt, timezone, { day: "2-digit", month: "short", year: "numeric" }) : "—"}
                  </td>
                  <td className="px-5 py-3 text-right font-semibold">{money(r._sum.priceCents ?? 0, currency)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="flex flex-col gap-2 border-t border-zinc-100 dark:border-zinc-800 px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            <span className="font-semibold text-zinc-700 dark:text-zinc-300">Totales</span> · {totalClients} clientes ·{" "}
            {totalAppointments} citas · {money(totalAmountCents, currency)}
          </p>
