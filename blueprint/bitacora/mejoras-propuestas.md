...
> ## ⚠️ CONTEXTO SAEFL/Laravel
>
> Este documento fue redactado para **SAEFL (sistema escolar) sobre Laravel 10 / Livewire / workers de cola** y su estado de implementación corresponde a ese proyecto. El proyecto actual es **BarberService (Next.js 16 / Prisma 7 / PostgreSQL)**, donde no existen workers `queue:work` ni supervisor; la vía asíncrona es `after()` de `next/server` y los jobs se ejecutan con Vercel Cron (ver Anexo C de `SPEC-BINNACLE-001.md`).
>
> Las mejoras se conservan como **catálogo de ideas** priorizadas por impacto/valor. Se marca su equivalencia funcional en BarberService; no aplicar tal cual los comandos/artisan ni las rutas Livewire.
