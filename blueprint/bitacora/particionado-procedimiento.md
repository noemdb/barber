...
> ## ⚠️ CONTEXTO SAEFL/MariaDB
>
> Este procedimiento está redactado para **SAEFL sobre MariaDB** (`ALTER TABLE ... PARTITION BY`, triggers de MariaDB, `artisan`). El proyecto actual es **BarberService (PostgreSQL/Neon)**, donde el particionado se haría con la sintaxis de PostgreSQL (`PARTITION BY RANGE (created_at)`, índices y triggers de Postgres) y se dispararía mediante un job/script, no `artisan binnacle:check-growth`.
>
> La lógica de decisión (proyectar crecimiento, umbral, swap sin downtime, mantener triggers de inmutabilidad) es **portable**; la sintaxis SQL y los comandos no. Ver Anexo C de `SPEC-BINNACLE-001.md`.
