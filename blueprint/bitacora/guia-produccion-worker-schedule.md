...
>
> Esta guía es específica de **SAEFL sobre Laravel** (worker de cola `binnacle` con Supervisor, `artisan schedule:run`, cron). En **BarberService (Next.js 16 / Prisma 7 / PostgreSQL, serverless)** no existe worker de cola ni Supervisor: la escritura no crítica se resuelve con `after()` de `next/server` y los jobs programados con **Vercel Cron** (cron.json + funciones on-demand). Ver Anexo C de `SPEC-BINNACLE-001.md`.
